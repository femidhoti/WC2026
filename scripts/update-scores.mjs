// ════════════════════════════════════════════════════════════════════
//  WC2026 hourly score updater (runs in GitHub Actions, server-side).
//  Pulls live results from TheSportsDB, computes scorelines / group
//  1st-2nd / knockout winners, and writes them to the Firebase Realtime
//  Database via REST. No browser needed, no CORS, no secrets (DB is in
//  open test mode through 2026-08-31).
// ════════════════════════════════════════════════════════════════════
const DB_URL='https://terry-worldcup26-default-rtdb.europe-west1.firebasedatabase.app';
const SPORTSDB_KEY='3'; // free TheSportsDB test key — swap for your own free key if rate-limited
const FEED='https://www.thesportsdb.com/api/v1/json/'+SPORTSDB_KEY+'/eventsseason.php?id=4429&s=2026';
...
function isFinished(ev){
  const s=(ev.strStatus||'').trim().toLowerCase();
  if(NOT_FINAL.has(s))return false;
  return ev.intHomeScore!=null&&ev.intHomeScore!==''&&ev.intAwayScore!=null&&ev.intAwayScore!=='';
}
